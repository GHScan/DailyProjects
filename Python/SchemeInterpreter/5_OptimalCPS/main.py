...
def eval(env, exp, k):
    if isinstance(exp, str):
        ...
        return k(lambda actuals, k2: evalSequentially(Env(env, dict(zip(formals, actuals))), expList, k2))
    elif exp[0] == 'begin':
        ...
        if isinstance(exp[1], str):
            (_, name, value) = exp
            return eval(env, value, lambda v: k(env.define(name, v)))
        else:
            name, formals, expList = exp[1][0], exp[1][1:], exp[2:]
            return eval(env, ['define', name, ['lambda', formals] + expList], k)
    elif exp[0] == 'set!':
        (_, name, value) = exp
        return eval(env, value, lambda v: k(env.set(name, v)))
    elif exp[0] == 'let':
...
def forceEval(env, exp):
    v, k = eval(env, exp, lambda v:(v, None))
    while k:
        v, k = k(v)
    return v

def funcsToCPS(funcs):
    return dict((name, lambda actuals, k, f=f: k(f(*actuals))) for name, f in funcs.iteritems())
