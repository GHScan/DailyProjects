...
def integral_linear_approximately(f, x1, x2, n = 10):
    dx = (x2 - x1) / n
    return dx * ((f(x1) + f(x2)) / 2 + sum(f(x1 + dx * i) for i in range(1, n)))

def integral_quadric_approximately(f, x1, x2, n = 10):
    dx = (x2 - x1) / (2 * n)
    return dx * (f(x1) + f(x2) 
...
def main():
    ...
    print(integral_linear_approximately(f, x1, x2))
    print(integral_quadric_approximately(f, x1, x2))
