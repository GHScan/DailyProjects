    ...
    internal class Program
    {
        private static void Main(string[] args)
        {
...

            {
                ...
                mainKernel.Compile();
                mainKernel.Run(env.CommandQueues.First(), mem, 64, -0.1, 0.651, -1.5, 1.5, -1.5, 1.5, (uint)height, (uint)width, 0, 0);

                env.CommandQueues.First().ReadFromBuffer(mem, buf);
            }

            var bm = new Bitmap(width, height, PixelFormat.Format32bppArgb);
            var data = bm.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);
