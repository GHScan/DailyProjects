...
private:
    ...
    public:
        ...
        bool moveNext() override final {
            if (mTake && mPreEnumerator->moveNext() && mF(mPreEnumerator->current())) return true;
            mTake = false;
            return false;
        }
        T current() override final {
...
public:
    class STLAdapterIterator {
    public:
        explicit STLAdapterIterator(IEnumeratorPtr<T> enumerator) : mEnumerator(enumerator) {
            if (mEnumerator) ++*this;
        }
        ...
        bool operator == (STLAdapterIterator const & o) {
            return mEnumerator == o.mEnumerator;
        }
        bool operator != (STLAdapterIterator const & o) {
            return !(*this == o);
        }
        STLAdapterIterator operator ++ (int) {
            STLAdapterIterator t = *this;
            ++*this;
            return t;
        }
        STLAdapterIterator& operator ++ () {
            if (!mEnumerator->moveNext()) mEnumerator = nullptr;
            return *this;
        }

    private:
        IEnumeratorPtr<T> mEnumerator;
    };

    STLAdapterIterator begin() {
        return STLAdapterIterator(mEnumerable->getEnumerator());
    }
    STLAdapterIterator end() {
        return STLAdapterIterator(nullptr);
    }
...
static void unitTest() {
    ...
    {
        int sum = 0;
        auto e = range(0, 10).filter([](int i){return i % 2 == 1; }).map([](int i){return i * 3; });
        for (auto v : e) {
            sum += v;
        }
        assert(sum == 75);

        for (auto v : range(0, 0)) puts("Invalid behavior");
    }
}
