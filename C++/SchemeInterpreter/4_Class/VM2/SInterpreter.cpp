...
static void setupFrame(
    ...
    frame->localEnv = objMgr->createObject<SEnv>(ScopedValue<SObject*>(frame->func->env), actualCount);
        ...
        while (pc < maxPc) {
            switch (codes[pc]) {
                case BCE_LoadBool:
                    ...
                    break;
                case BCE_LoadInt:
                    evalStack->push(SValue(reinterpret_cast<ByteCode<BCE_LoadInt>*>(&codes[pc])->value));
                ...
                case BCE_StoreGlobal:
                    ...
                    break;
                case BCE_LoadLocal:
                    evalStack->push(frame->localEnv->getValue(reinterpret_cast<ByteCode<BCE_LoadLocal>*>(&codes[pc])->lindex));
                    pc += sizeof(ByteCode<BCE_LoadLocal>);
                    break;
                case BCE_LoadLocal0:
                    evalStack->push(frame->localEnv->getValue(0));
                    pc += sizeof(ByteCode<BCE_LoadLocal0>);
                    break;
                case BCE_LoadLocal1:
                    evalStack->push(frame->localEnv->getValue(1));
                    pc += sizeof(ByteCode<BCE_LoadLocal1>);
                    break;
                case BCE_LoadLocal2:
                    evalStack->push(frame->localEnv->getValue(2));
                    pc += sizeof(ByteCode<BCE_LoadLocal2>);
                    break;
                case BCE_StoreLocal:
                    frame->localEnv->setValue(reinterpret_cast<ByteCode<BCE_StoreLocal>*>(&codes[pc])->lindex, evalStack->pop());
                    pc += sizeof(ByteCode<BCE_StoreLocal>);
                    break;
                case BCE_LoadFree: {
                    auto ins = reinterpret_cast<ByteCode<BCE_LoadFree>*>(&codes[pc]);
                    evalStack->push(frame->localEnv->getUpEnv(ins->envIndex)->getValue(ins->index));
                    pc += sizeof(ByteCode<BCE_LoadFree>);
                   }
                    break;
                case BCE_LoadFree1:
                    evalStack->push(frame->localEnv->prevEnv->getValue(reinterpret_cast<ByteCode<BCE_LoadFree1>*>(&codes[pc])->index));
                    pc += sizeof(ByteCode<BCE_LoadFree1>);
                    break;
                case BCE_LoadFree2:
                    evalStack->push(frame->localEnv->prevEnv->prevEnv->getValue(reinterpret_cast<ByteCode<BCE_LoadFree2>*>(&codes[pc])->index));
                    pc += sizeof(ByteCode<BCE_LoadFree2>);
                    break;
                case BCE_LoadFree3:
                    evalStack->push(frame->localEnv->prevEnv->prevEnv->prevEnv->getValue(reinterpret_cast<ByteCode<BCE_LoadFree3>*>(&codes[pc])->index));
                    pc += sizeof(ByteCode<BCE_LoadFree3>);
                    break;
                case BCE_StoreFree: {
                    auto ins = reinterpret_cast<ByteCode<BCE_StoreFree>*>(&codes[pc]);
                    frame->localEnv->getUpEnv(ins->envIndex)->setValue(ins->index, evalStack->pop());
                    pc += sizeof(ByteCode<BCE_StoreFree>);
                }
                    break;
                case BCE_StoreFree1: {
                    frame->localEnv->prevEnv->setValue(reinterpret_cast<ByteCode<BCE_StoreFree1>*>(&codes[pc])->index, evalStack->pop());
                    pc += sizeof(ByteCode<BCE_StoreFree1>);
                 }
                    break;
                case BCE_StoreFree2: {
                    frame->localEnv->prevEnv->prevEnv->setValue(reinterpret_cast<ByteCode<BCE_StoreFree2>*>(&codes[pc])->index, evalStack->pop());
                    pc += sizeof(ByteCode<BCE_StoreFree2>);
                 }
                    break;
                case BCE_StoreFree3: {
                    frame->localEnv->prevEnv->prevEnv->prevEnv->setValue(reinterpret_cast<ByteCode<BCE_StoreFree3>*>(&codes[pc])->index, evalStack->pop());
                    pc += sizeof(ByteCode<BCE_StoreFree3>);
                 }
                    break;
                case BCE_LoadFunc:
                    evalStack->push(
                            SValue(objMgr->createObject<SFunc>(ScopedValue<SObject*>(frame->localEnv), fprotos[reinterpret_cast<ByteCode<BCE_LoadFunc>*>(&codes[pc])->findex])));
                ...
                case BCE_Call: {
                        ...
                        switch (pfunc->getType()) {
                            ...
                            case SVT_Class: {
                                ...
                                auto obj = objMgr->createObject<SEnv>(ScopedValue<SObject*>(sclass->asEnv()), actualCount);
                                *pfunc = SValue(obj);
                                {
                                    auto iter = pfunc;
                                    for (int i = 0; i < actualCount; ++i) {
                ...
                case BCE_LoadClass:
                    evalStack->push(
                            SValue(objMgr->createObject<SClass>(ScopedValue<SObject*>(frame->localEnv), cprotos[reinterpret_cast<ByteCode<BCE_LoadClass>*>(&codes[pc])->cindex])));
                    pc += sizeof(ByteCode<BCE_LoadClass>);
                    break;
                case BCE_LoadMethod: {
                    auto ins = reinterpret_cast<ByteCode<BCE_LoadMethod>*>(&codes[pc]);
                    auto sclass = reinterpret_cast<SClass*>(frame->localEnv->getUpEnv(ins->envIndex));
                ...
                case BCE_LoadCachedMethod: {
                    ...
                    evalStack->push(
                            SValue(objMgr->createObject<SFunc>(ScopedValue<SObject*>(frame->localEnv->getUpEnv(ins->cachedObjIndex)), ins->cachedFunc)));
                ...
                case BCE_GetCachedMethod: {
                    ...
                    auto obj = evalStack->top<-1>().getObject<SEnv>();
                    if (reinterpret_cast<SClass*>(obj->prevEnv)->proto == ins->cachedClass) {
                        evalStack->top<-1>() = SValue(objMgr->createObject<SFunc>(ScopedValue<SObject*>(obj), ins->cachedFunc));
                ...
                case BCE_Inline_Cons: {
                    evalStack->top<-2>() = SValue(objMgr->createObject<SPair>(
                                ScopedValue<SValue>(evalStack->top<-2>()), ScopedValue<SValue>(evalStack->top<-1>())));
                    evalStack->pop();
                    if (frame == nullptr) goto Label_PeekFrame;
