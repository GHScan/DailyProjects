...
void SInterpreterImpl::defineBuiltin(const char *name, CFunction f) {
    mObjMgr->createCFunction(allocBuiltin(name), f);
}
...
void SInterpreterImpl::setupBuiltin() {
    ...
    defineBuiltin("+", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        SNumeric::add(mgr, &stack->top(-3), stack->top(-2), stack->top(-1));
    });
    defineBuiltin("-", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        SNumeric::sub(mgr, &stack->top(-3), stack->top(-2), stack->top(-1));
    });
    defineBuiltin("*", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        SNumeric::mul(mgr, &stack->top(-3), stack->top(-2), stack->top(-1));
    });
    defineBuiltin("/", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        SNumeric::div(mgr, &stack->top(-3), stack->top(-2), stack->top(-1));
    });
    defineBuiltin("quotient", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        SNumeric::quotient(mgr, &stack->top(-3), stack->top(-2), stack->top(-1));
    });
    defineBuiltin("remainder", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        SNumeric::remainder(mgr, &stack->top(-3), stack->top(-2), stack->top(-1));
    });

    defineBuiltin("=", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        SNumeric::equal(mgr, &stack->top(-3), stack->top(-2), stack->top(-1));
    });
    defineBuiltin("<", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        SNumeric::less(mgr, &stack->top(-3), stack->top(-2), stack->top(-1));
    });
    defineBuiltin("<=", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        SNumeric::lessEqual(mgr, &stack->top(-3), stack->top(-2), stack->top(-1));
    });
    defineBuiltin(">", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        SNumeric::greater(mgr, &stack->top(-3), stack->top(-2), stack->top(-1));
    });
    defineBuiltin(">=", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        SNumeric::greaterEqual(mgr, &stack->top(-3), stack->top(-2), stack->top(-1));
    });

    defineBuiltin("not", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        SNumeric::_not(mgr, &stack->top(-2), stack->top(-1));
    });
    defineBuiltin("sqr", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        SNumeric::sqr(mgr, &stack->top(-2), stack->top(-1));
    });
    defineBuiltin("sqrt", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        SNumeric::sqrt(mgr, &stack->top(-2), stack->top(-1));
    });
    ...
    defineBuiltin("eq?", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        mgr->createBool(&stack->top(-3), stack->top(-2) == stack->top(-1));
    });
    defineBuiltin("equal?", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        mgr->createBool(&stack->top(-3), stack->top(-2).equal(stack->top(-1)));
    });

    defineBuiltin("cons", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        mgr->createPair(&stack->top(-3), stack->top(-2), stack->top(-1));
    });
    ...
    defineBuiltin("drop", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        SPairListAccessor list(stack->top(-2).getObject()->staticCast<SPair>());
    ...
    defineBuiltin("append", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        SPairListAccessor firstList(stack->top(-2).getObject()->staticCast<SPair>());
        SPairListBuilder retList(mgr, &stack->top(-3));
    ...
    defineBuiltin("length", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        SPairListAccessor list(stack->top(-1).getObject()->staticCast<SPair>());
        mgr->createInt(&stack->top(-2), distance(list.begin(), list.end()));
    });
    ...
    defineBuiltin("empty?", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        mgr->createBool(&stack->top(-2), stack->top(-1) == SValue::EMPTY);
    });
    ...
    defineBuiltin("current-inexact-milliseconds", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        mgr->createDouble(&stack->top(-1), clock() * 1000.0 / CLOCKS_PER_SEC);
    });
    defineBuiltin("random", [](SObjectManager *mgr, SEvalStack *stack, int actualCount){
        mgr->createInt(&stack->top(-2), ::rand() % stack->top(-1).getInt());
    });
...
void SInterpreterImpl::interpret(istream &si) {
    ...
    parse(mObjMgr, mEvalStack->alloc(), source);
...
void SInterpreterImpl::eval() {
    ...
    mObjMgr->createScriptFunction(&mEvalStack->top(-1), mProtos.back().get(), nullptr);
