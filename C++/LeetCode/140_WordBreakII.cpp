...
class Solution {
public:
    vector<string> wordBreak(string const &s, vector<string> const &wordDict) {
        ...
        vector<int> wordBreak;
        enumerateWordBreak(result, wordBreak, (int)s.size(), 0, map, wordDict);
        ...
        vector<string> const &wordDict)
    {
        ...
        {
            string s;
            for (int i = 0; i < (int)wordBreak.size(); ++i)
            {
                if (i > 0)
                    s.push_back(' ');
                string const &word = wordDict[wordBreak[i]];
                s.insert(s.end(), word.begin(), word.end());
            }
        ...
        {
            wordBreak.push_back(it->second);

            enumerateWordBreak(
                result, wordBreak, len, i + (int)wordDict[it->second].size(), tokens, wordDict);
