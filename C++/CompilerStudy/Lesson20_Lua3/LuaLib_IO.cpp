...
static void file_read(const vector<LuaValue>& args, vector<LuaValue>& rets) {
    ...
    if (args.size() > 1 && args[1].isTypeOf(LVT_Number)) {
        ...
        vector<char> s(n + 1, 0);
        n = (int)fread(&s[0], 1, n, f);
        if (n > 0) {
            s.resize(n);
            rets.push_back(LuaValue(&s[0], (int)s.size()));
        } else rets.push_back(LuaValue::NIL);
        return;
    }

    string fmt = "*a";
    if (args.size() > 1) fmt = args[1].getString()->buf();
    if (fmt == "*a") {
        long off = ftell(f);
        fseek(f, 0, SEEK_END);
        long off2 = ftell(f); 
        ...
        if (off2 > off) {
            vector<char> buf(off2 - off + 1, 0);
            if (fread(&buf[0], 1, buf.size() - 1, f) > 0)  {
                rets.push_back(LuaValue(&buf[0], (int)buf.size()));
            } else rets.push_back(LuaValue::NIL);
        } else rets.push_back(LuaValue::NIL);
...
static void file_seek(const vector<LuaValue>& args, vector<LuaValue>& rets) {
    ...
    string where = "cur";
    int off = 0;
    if (args.size() >= 2) where = args[1].getString()->buf();
...
static LuaTable* getFileMetaTable() {
    auto ioTable = LuaVM::instance()->getGlobalTable()->get(LuaValue("io")).getTable();
...
static void io_close(const vector<LuaValue>& args, vector<LuaValue>& rets) {
    ...
    if (_args.empty()) {
        _args.push_back(
                LuaVM::instance()->getGlobalTable()->get(LuaValue("io")).getTable()->get(LuaValue(IOFIELD_ODEFAULT)));
    } 
...
static void io_input(const vector<LuaValue>& args, vector<LuaValue>& rets) {
    auto r = LuaVM::instance()->getGlobalTable()->get(LuaValue("io")).getTable()->get(LuaValue(IOFIELD_IDEFAULT));
    if (!args.empty()) {
        LuaValue f(args[0]);
        if (f.isTypeOf(LVT_String)) {
            f = LuaValue(openFile(f.getString()->buf(), "r"));
        }
        ASSERT(f.isTypeOf(LVT_Table));
        LuaVM::instance()->getGlobalTable()->get(LuaValue("io")).getTable()->set(LuaValue(IOFIELD_IDEFAULT), f);
    }
    rets.push_back(r);
}
static void io_lines(const vector<LuaValue>& args, vector<LuaValue>& rets) {
    ...
    if (args.empty()) {
        _args.push_back(
                LuaVM::instance()->getGlobalTable()->get(LuaValue("io")).getTable()->get(LuaValue(IOFIELD_IDEFAULT)));
    } else {
        _args.push_back(LuaValue(openFile(args[0].getString()->buf(), "r")));
    }
    file_lines(_args, rets);
}
static void io_open(const vector<LuaValue>& args, vector<LuaValue>& rets) {
    const char *fname = args[0].getString()->buf();
    const char *mode = "r";
    if (args.size() >= 2) mode = args[1].getString()->buf();
...
static void io_output(const vector<LuaValue>& args, vector<LuaValue>& rets) {
    auto r = LuaVM::instance()->getGlobalTable()->get(LuaValue("io")).getTable()->get(LuaValue(IOFIELD_ODEFAULT));
    if (!args.empty()) {
        LuaValue f(args[0]);
        if (f.isTypeOf(LVT_String)) {
            f = LuaValue(openFile(f.getString()->buf(), "w"));
        }
        ASSERT(f.isTypeOf(LVT_Table));
        LuaVM::instance()->getGlobalTable()->get(LuaValue("io")).getTable()->set(LuaValue(IOFIELD_ODEFAULT), f);
    }
    rets.push_back(r);
}
...
static void io_read(const vector<LuaValue>& args, vector<LuaValue>& rets) {
    vector<LuaValue> _args(args);
    auto file = LuaVM::instance()->getGlobalTable()->get(LuaValue("io")).getTable()->get(LuaValue(IOFIELD_IDEFAULT));
...
static void io_type(const vector<LuaValue>& args, vector<LuaValue>& rets) {
    ...
    if (arg.isTypeOf(LVT_Table)) {
        ...
        if (type.isTypeOf(LVT_String) && type.getString()->buf() == string(OBJ_TYPE)) {
            auto f = (FILE*)table->get(LuaValue(NumberType(1))).getLightUserData();
...
static void io_write(const vector<LuaValue>& args, vector<LuaValue>& rets) {
    vector<LuaValue> _args(args);
    auto file = LuaVM::instance()->getGlobalTable()->get(LuaValue("io")).getTable()->get(LuaValue(IOFIELD_ODEFAULT));
    _args.insert(_args.begin(), file);
...
extern void openLib_io() {
    auto table = LuaTable::create();
    LuaVM::instance()->getGlobalTable()->set(LuaValue("io"), LuaValue(table));
