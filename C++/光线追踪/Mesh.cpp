...
void Mesh::optimize()
{
    for (int i = 0; i < (int)m_subMeshs.size(); ++i) {
        m_subMeshs[i]->optimize();
    }
}
void Mesh::printInfo(FILE *f) const
{
    fprintf(f, "mesh - %s : \n", m_fname.c_str());
    for (int i = 0; i < getSubCount(); ++i) {
        const SubMesh *p = sub(i);
        fprintf(f, "    sub %d : \n", i);
        fprintf(f, "        vertex count : %d\n", p->vertexBuffer.getVertexCount());
        fprintf(f, "        triangle count : %d\n", p->indexBuffer.getTriangleCount());
    }
}

const std::string Mesh::getFileName() const { return m_fname; }
void Mesh::setFileName(const std::string& fname) { m_fname = fname; }
...
const Mesh* MeshManager::getMesh(const std::string& fname)
{
    assert(!fname.empty());
    {
        Mesh *p = BuildinMeshManager::instance()->getMesh(fname);
        if (p != NULL) return p;
    }
