...
static void* memcpy_avx(void* destination, const void* source, size_t num) {
    auto dst = static_cast<uint8_t*>(destination);
    auto src = static_cast<uint8_t const*>(source);
...
_label_switch:
    ...
    {
        auto a = _mm256_loadu_si256(reinterpret_cast<__m256i const*>(src));
        auto b = _mm_loadu_si128(reinterpret_cast<__m128i const*>(src) + 2);
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(dst), a);
        _mm_storeu_si128(reinterpret_cast<__m128i*>(dst) + 2, b);
        dst += 48;
        src += 48;
        num -= 48;
        goto _label_switch;
