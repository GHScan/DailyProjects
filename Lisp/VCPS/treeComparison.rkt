...
;------------------------------
; tree -> stream
(define (tree-equal-stream? t1 t2)
  (define (stream-equal? a b)
    (cond
      [(stream-empty? a) (stream-empty? b)]
      [(stream-empty? b) (stream-empty? a)]
      [(equal? (stream-first a) (stream-first b)) (stream-equal? (stream-rest a) (stream-rest b))]
      [else false])
    )
  (define (generate-leaves t)
    (cond
      [(empty? t) empty-stream]
      [(pair? t) (stream-append (generate-leaves (car t)) (generate-leaves (cdr t)))]
      [else (stream t)])
    )
  (stream-equal? (generate-leaves t1) (generate-leaves t2))
  )
...
(define (run-benchmark)
  (let ([datas (list
                 `(100-same ,@((lambda (x) (list x x)) (construct-random-tree (range 100))))
                 `(1000-same ,@((lambda (x) (list x x)) (construct-random-tree (range 1000))))
                 `(100-diff-first ,@((lambda (x) (list (cons 1 x) (cons 2 x))) (construct-random-tree (range 100))))
                 `(1000-diff-first ,@((lambda (x) (list (cons 1 x) (cons 2 x))) (construct-random-tree (range 1000))))
                 `(100-random ,@(construct-random-tree (range 100)) ,@(construct-random-tree (range 100)))
                 `(1000-random ,@(construct-random-tree (range 1000)) ,@(construct-random-tree (range 1000)))
                 )])
