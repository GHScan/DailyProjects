...
(define truncatable-primes
  (let iter ([left-primes init-primes][right-primes init-primes]
             [result empty]
             [left-expander (lambda (prime digit) (cons digit prime))][right-expander (lambda (prime digit) (append prime (list digit)))]
             [left-digits (range 1 10)][right-digits '(1 3 5 7 9)])
    (if (or (empty? left-primes) (empty? right-primes) (>= (length result) 15))
      result
      (iter 
        ...
        (append (list-intersect (map list->number left-primes)
                ...
                result)
        left-expander right-expander left-digits right-digits))
    ))
