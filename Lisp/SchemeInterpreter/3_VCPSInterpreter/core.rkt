...
(define (compile exp)
  (cond 
    [(or (string? exp) (number? exp)) 
     (lambda (env k) (cons k exp))]
    [(symbol? exp) 
     (let ([loc (make-varaible-location exp)])
       (lambda (env k) (cons k (variable-location-read loc env))))]
    [else 
      (match 
        exp
        [(list 'quote e)
         (lambda (env k) (cons k e))]
        ...
        [(list 'lambda args exp-list ...)
         (let ([exp-list (compile (cons 'begin exp-list))][name-vec (find-definitions exp-list args)])
           (lambda (env k)
             (cons k (make-script-procedure env args exp-list name-vec))))
         ]
...
(define (eval env exp)
  (do ([cv ((compile exp) env (lambda (v) (cons empty v))) ((car cv) (cdr cv))])
    ((empty? (car cv)) (cdr cv)))
  )
;------------------------------
