...
type SkipList<'k, 'v when 'k : comparison>(height : int) =
    ...
    let newNodeHeight (maxHeight : int) (random : System.Random) =
        let rec loop height randomNum = 
            if height < maxHeight && randomNum % 2 = 0 then
                loop (height + 1) (randomNum / 2)
            else
                height
        loop 1 (random.Next())
    ...
    override this.ToString() = sprintf "SkipList(%A)" ([for i in 1..height -> getNodeCount head (i - 1)])
...
type SkipListTest() =
    ...
    member this.TestRandom() =
        ...
        for i in 0..kCount do 
            ...
            if not (dict.ContainsKey(num)) then
                dict.Add(num, num.ToString())
            if not (sl.Contains(num)) then
                sl.Add(num, num.ToString()) |> ignore
            Assert.AreEqual(dict.Count, sl.Count)

        for i in 0..kCount do
            let num = random.Next(0, kCount)
            if dict.ContainsKey(num) then
                dict.Remove(num) |> ignore
            if sl.Contains(num) then
                sl.Remove(num) |> ignore
            Assert.AreEqual(dict.Count, sl.Count)
